    ...
    <article class="card mb-4">
        <div class="card-body bg-gray-100">
            ...
            <form wire:submit.prevent="update">
                <input wire:model="audience.name" type="text" class="block w-full mt-1 shadow-sm border-gray-300 rounded" placeholder="Ingrese una meta">
                @error('audience.name')
    ...
    <article class="card">
        <div class="card-body bg-gray-100">
            <form wire:submit.prevent="store">
                <input type="text" class="flex-1 mt-1 shadow-sm border-gray-300 rounded w-full" wire:model="name" placeholder="Igrese la audiencia del curso">
                @error('name')
                <span class="text-red-600 text-sm">{{$message}}</span>
                @enderror
                <div class="flex justify-end mt-2">
                    <button class="btn btn-primary">Agregar audiencia</button>
                </div>
            </form>
        </div>
    </article>
